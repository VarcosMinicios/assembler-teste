...
runner_1.test.group('Configure Encore', (group) => {
    group.each.teardown(async () => {
        await fs.cleanup();
    });
    (0, runner_1.test)('setup encore', async ({ assert }) => {
        ...
        await invoke.run();
        const envFile = await fs.fsExtra.pathExists((0, path_1.join)(fs.basePath, 'webpack.config.js'));
        const envExampleFile = await fs.fsExtra.readFile((0, path_1.join)(fs.basePath, 'resources/js/app.js'), 'utf-8');
