...
const Exception_1 = __importDefault(require("../commands/Make/Exception"));
...
runner_1.test.group('Make Exception', (group) => {
    group.setup(() => {
    ...
    (0, runner_1.test)('make an exception class inside the default directory', async ({ assert }) => {
        ...
        const exception = new Exception_1.default(app, new ace_1.Kernel(app).mockConsoleOutput());
        exception.name = 'user';
        await exception.run();
        ...
        assert.deepEqual((0, test_helpers_1.toNewlineArray)(UserException), (0, test_helpers_1.toNewlineArray)(ExceptionTemplate.replace(new RegExp('\\{{ filename }}', 'g'), 'UserException')));
    });
    (0, runner_1.test)('make a self-handled exception class inside the default directory', async ({ assert }) => {
        await fs.add('.adonisrc.json', JSON.stringify({}));
        const rcContents = (0, fs_extra_1.readJSONSync)((0, path_1.join)(fs.basePath, '.adonisrc.json'));
        const app = new application_1.Application(fs.basePath, 'test', rcContents);
        const exception = new Exception_1.default(app, new ace_1.Kernel(app).mockConsoleOutput());
    ...
    (0, runner_1.test)('make an exception class inside a custom directory', async ({ assert }) => {
        await fs.add('.adonisrc.json', JSON.stringify({
            namespaces: {
